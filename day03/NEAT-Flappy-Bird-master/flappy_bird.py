...
class Pipe():
    ...
    def __init__(self, x, gap=MAX_GAP):
        """
...
def eval_genomes(genomes, config):
    ...
    BASE_VEL = 5
    MAX_VEL = 10
    SCORE_PER_VEL_INCREASE = 20  # 每20分速度增加0.5
    VEL_INCREMENT = 0.5

    SCORE_PER_GAP_REDUCTION = 10  # 每10分间隙减少5
    GAP_REDUCTION_STEP = 5
    MIN_GAP = 100
    INIT_GAP = 200
    ...
    while run and len(birds) > 0:
        ...
        # ---------- 动态调整难度 ----------
        # 1. 更新移动速度（所有水管和地面同步）
        new_vel = BASE_VEL + (score // SCORE_PER_VEL_INCREASE) * VEL_INCREMENT
        new_vel = min(MAX_VEL, new_vel)
        Pipe.VEL = new_vel
        Base.VEL = new_vel  # 地面速度保持一致

        # 2. 计算当前分数对应的间隙（用于新生成的水管）
        current_gap = INIT_GAP - (score // SCORE_PER_GAP_REDUCTION) * GAP_REDUCTION_STEP
        current_gap = max(MIN_GAP, current_gap)
        # --------------------------------
        ...
        if add_pipe:
            ...
            for genome in ge:
                genome.fitness += 5
            # 使用当前难度下的间隙创建新水管
            pipes.append(Pipe(WIN_WIDTH, gap=current_gap))

        for r in rem:
            pipes.remove(r)

            # 边界死亡判断
            for bird in birds[:]:  # 使用切片副本遍历
